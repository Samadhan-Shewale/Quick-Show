...
function SeatLayouts() {
    ...
    const handleSeatClick = ( seatId ) => {
        // select the time first 
        if(!selectedTime ){
            return toast("Please select time first ");
        }

        if(!selectedSeats.includes(seatId) && selectedSeats.length > 4 ){
            return toast("Only can only select 5 seats ");
        }
    ...
    const handleCheckout = () =>{
        ...
        if(selectedSeats.length <= 0  ){
            return toast("Please select seat first ");
        }else{
    ...
    return show ? ( 
        <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50 '>
            {/* Available time  */}
            <div className='w-50 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max 
            md:sticky md:top-30 '> 
                <p className='text-lg font-semibold px-6 '>Available Timings </p>
                <div className='mt-5 space-y-1 '>
                    { show.dateTime[date].map( (item)=>( 
                        <div onClick={ ()=> setSelectedTime(item) }
                        key={item.date} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md 
            ...
            <div className='relative flex-1 flex flex-col items-center max-md:mt-16 '>
                    ...
                    <button onClick={ ()=> handleCheckout() } className='flex items-center gap-1 mt-20 px-10 py-3 text-sm bg-primary
